...
class RiverSwim(FinitePOMDP):
    ...
    @staticmethod
    def build_transition_kernel_static(params: ArrayLike, n_states: int, n_actions: int = 2) -> NDArray:
        """
        ...
        left = 0
        right = 1
        transit_probs = np.zeros(batch_shape + (n_states, n_actions, n_states))
        
        # Action "Left" at state s always leads to s' = max(s-1, 0) 
        states = np.arange(n_states)
        next_states = np.clip(states - 1, a_min=0, a_max=n_states-1)
        transit_probs[..., states, left, next_states] = 1.0

        # Action "Right":
        #   if s = 0, s' = 0 w.p. p1 and s' = 1 w.p. 1 - p1
        transit_probs[..., 0, right, 0] = p1
        transit_probs[..., 0, right, 1] = p2 + p3
        #   if s = L-1, s' = L-1 w.p. p1 and s'= L-2 w.p. 1 - p1
        transit_probs[..., -1, right, -1] = p1
        transit_probs[..., -1, right, -2] = p2 + p3
        #   if s = 1, ..., L-2:
        #       s' = s w.p. p1
        #       s' = s + 1 w.p. p2
        #       s' = s - 1 w.p. p3
        if n_states > 2:
            ...
            transit_probs[..., states, right, states] = p1[..., np.newaxis] if batch_shape else p1
            transit_probs[..., states, right, states + 1] = p2[..., np.newaxis] if batch_shape else p2
            transit_probs[..., states, right, states - 1] = p3[..., np.newaxis] if batch_shape else p3
    ...
    @staticmethod
    def build_reward_matrix_static(params: ArrayLike, n_obs: int, n_actions: int = 2) -> NDArray:
        """
    ...
    def __init__(self, river_length: int, horizon: int, params: Dict[str, Any], discount: float = 1.0):
        super().__init__(
